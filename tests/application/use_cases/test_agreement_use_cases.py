...
@pytest.mark.asyncio
async def test__validate_agreement_business_rules_exception():
    class Repo(AsyncMock):
        async def exists_agreement_by_number_and_business_unit(self, number, bu):
            raise Exception("fail")
    use_cases = AgreementUseCases(Repo())
    agreement_data = DummyAgreement()
    agreement_data.agreement_number = "A-003"
    agreement_data.business_unit_id = 1
    with pytest.raises(ValueError):
        await use_cases._validate_agreement_business_rules(agreement_data)

@pytest.mark.asyncio
async def test__build_agreement_response_empty_lists():
    repo = AsyncMock()
    use_cases = AgreementUseCases(repo)
    agreement = DummyAgreement()
    products = []
    store_rules = []
    excluded_flags = []
    use_cases.log_info = MagicMock()
    with pytest.raises(ValueError):
        await use_cases._build_agreement_response(agreement, products, store_rules, excluded_flags)

@pytest.mark.asyncio
async def test__build_agreement_response_exception():
    repo = AsyncMock()
    use_cases = AgreementUseCases(repo)
    agreement = DummyAgreement()
    use_cases.log_info = MagicMock()
    # Forzamos un error en la construcción de productos
    with pytest.raises(Exception):
        await use_cases._build_agreement_response(agreement, [object()], [], [])

@pytest.mark.asyncio
async def test__validate_agreement_business_rules_for_update_exception():
    repo = AsyncMock()
    use_cases = AgreementUseCases(repo)
    agreement_data = AsyncMock()
    agreement_data.products = []
    agreement_data.store_rules = []
    agreement_data.excluded_flags = []
    use_cases._agreement_repository.exists_agreement_by_number_and_business_unit = AsyncMock(side_effect=Exception("fail"))
    # No debe lanzar ValueError, solo ejecutarse
    await use_cases._validate_agreement_business_rules_for_update(1, agreement_data)

@pytest.mark.asyncio
async def test__validate_agreement_business_rules_for_update_success():
    repo = AsyncMock()
    use_cases = AgreementUseCases(repo)
    agreement_data = AsyncMock()
    agreement_data.products = [AsyncMock(sku_code="A"), AsyncMock(sku_code="B")]
    agreement_data.store_rules = []
    agreement_data.excluded_flags = [AsyncMock(excluded_flag_id=1), AsyncMock(excluded_flag_id=2)]
    use_cases._agreement_repository.exists_agreement_by_number_and_business_unit = AsyncMock(return_value=False)
    await use_cases._validate_agreement_business_rules_for_update(1, agreement_data)

@pytest.mark.asyncio
async def test_create_agreement_log_error():
    repo = AsyncMock()
    use_cases = AgreementUseCases(repo)
    agreement_data = DummyAgreement()
    user = DummyUser()
    repo.create_complete_agreement.side_effect = Exception("fail")
    use_cases.log_error = MagicMock()
    with pytest.raises(Exception):
        await use_cases.create_agreement(agreement_data, user)
    assert use_cases.log_error.called

@pytest.mark.asyncio
async def test_create_agreement_log_warning():
    repo = AsyncMock()
    use_cases = AgreementUseCases(repo)
    agreement_data = DummyAgreement()
    user = DummyUser()
    repo.create_complete_agreement.side_effect = ValueError("fail")
    use_cases.log_warning = MagicMock()
    with pytest.raises(ValueError):
        await use_cases.create_agreement(agreement_data, user)
    assert use_cases.log_warning.called

@pytest.mark.asyncio
async def test__validate_agreement_business_rules_log_error():
    class Repo(AsyncMock):
        async def exists_agreement_by_number_and_business_unit(self, number, bu):
            raise Exception("fail")
    use_cases = AgreementUseCases(Repo())
    agreement_data = DummyAgreement()
    agreement_data.agreement_number = "A-004"
    agreement_data.business_unit_id = 1
    use_cases.log_error = MagicMock()
    with pytest.raises(ValueError):
        await use_cases._validate_agreement_business_rules(agreement_data)
